...
void benchmark_triad(const unsigned long N, const long long repeat, std::ofstream &myfile)
{
  ...
  myfile << N;
  myfile << " ";
  myfile << 1e-9 * 3 * sizeof(float) * N / best;
  myfile << std::endl;
...
int main(int argc, char **argv)
{
  ...
  std::ofstream myfile;
  myfile.open ("stream_triad_o3_uppmax.csv");

  for (long n = N_min; n <= N_max; n = (1 + n * 1.1))
    {
      // round up to nearest multiple of 8
      n = (n + 7) / 8 * 8;
      benchmark_triad(n, repeat, myfile);
    }
