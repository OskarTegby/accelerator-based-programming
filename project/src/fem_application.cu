...
template <typename Number>
Number run_test(const long long N, const long long n_repeat)
{
  ...
  SparseMatrix<Number> matrix_dev = matrix.copy_to_device(); // This call is new

  Vector<Number>    src(N * N * N, host_space); // This call is changed
...
#pragma omp parallel for collapse(2)
  ...
  // This replaces "matrix.apply(src, dst)"
  {
    double error = 0;
...
#pragma omp critical
  ...
  {
    cudaDeviceSynchronize(); // This call is new
    const auto t1 = std::chrono::steady_clock::now();
    for (unsigned long long rep = 0; rep < n_repeat; ++rep)
      matrix_dev.apply(src_device, dst_device); // This call is changed
    cudaDeviceSynchronize(); // This call is new
    ...
    throughput = 1e-9 * n_repeat * (matrix.memory_consumption() + 2 * dst.size() * sizeof(Number)) / time;
...

  {
    ...
    result(0) = 1.;
    result(1) = 0.8;
    Vector<Number> result_device = result.copy_to_device(); // This call is new

    const auto t1 = std::chrono::steady_clock::now();
    const auto info = solve_with_conjugate_gradient(500, // This call is changed
                                                    1e-12,
                                                    matrix_dev,
                                                    dst_device,
                                                    result_device);
    ...
    mupds = 1e-6 * dst.size() * info.first / time;
    ...
    result = result_device.copy_to_host(); // This call is new
    result.add(-1., src);
...
int main(int argc, char **argv)
{
  ...
  if (N == -1)
    for (unsigned long long NN = 24; NN < 256; NN += 24)
      {
        ...
        if (number == "double")
          myfile << run_test<double>(NN, n_repeat);
        else
          myfile << run_test<float>(NN, n_repeat);
  ...
  else
    {
      ...
      if (number == "double")
        myfile << run_test<double>(N, n_repeat);
      else
        myfile << run_test<float>(N, n_repeat);
